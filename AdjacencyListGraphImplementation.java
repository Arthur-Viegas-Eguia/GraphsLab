...
/**
 * Implements a graph using the adjacency list implementation with its basic
 * methods. 
...
public class AdjacencyListGraphImplementation implements UnweightedGraph {
    ...
    public boolean hasEdge(int begin, int end) {
        for(int i = 0; i < adjacencyLists.get(begin).size(); i++){
            if(adjacencyLists.get(begin).get(i) == end){
                return true;
            }
        }
        return false;
     ...
     * It allows the client to change the graph structure. 
     */
    @Override
    ...
    public boolean addEdge(int begin, int end) {
        if(begin+1 > adjacencyLists.size() || begin < 0 || end < 0){
            throw new IndexOutOfBoundsException();
        }
        for(int i = 0; i < adjacencyLists.get(begin).size(); i++){
            if(adjacencyLists.get(begin).get(i) == end){
        ...
        adjacencyLists.get(begin).add(end);
        if(!directed){
            adjacencyLists.get(end).add(begin);
        }
        return true;
    }
